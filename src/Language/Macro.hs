module Language.Macro where

import Data.IORef
import Control.Monad.Error

import Language.LispVal
import Language.LispError
...
evalMacro (env, macroEnv) (List (Atom "macro" : List (Atom var : params) : body)) = 
    makeNormalMacro env params body >>= defineVar macroEnv var >>= (\v -> return $ List [Atom "quote", v])
evalMacro (env, macroEnv) (List (Atom "macro" : DottedList (Atom var : params) varargs : body)) =
    makeVarArgsMacro varargs env params body >>= defineVar macroEnv var >>= (\v -> return $ List [Atom "quote", v])
